...
The package depends on `@peasant-labs/schema@0.1.0-rc10`; package managers install
...
shown when `canCopyLink` and a `sessionLinkBuilder` are present; `onCopyLink` is
optional because the component also attempts to write the built URL through the
Clipboard API. Other actions require their capability and callback, except
download, which has a built-in serializer. The package also exports
`ShareDialog`; hosts still own app-specific routes, authentication, queries, and
the flows they connect to these generic surfaces.
...
- **Views:** `HighlightsView`, `DiffsView`, `diffAnchorId`, `FilesView`,
  `AnnotationsView`, and `SessionScorecard`.
...
- **Schema and view types:** selected schema types (`SessionDetailPayload`,
  `TurnDetail`, `ToolCallDetail`, `SessionScorecard`, `QualitySession`, `Harness`,
  `Role`), browser-owned view envelopes (`Phase`, `PhaseBadge`, `PhaseType`),

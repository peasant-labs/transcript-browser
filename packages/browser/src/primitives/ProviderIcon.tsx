 ...
 *
 * Each glyph draws with `currentColor`. With `tone="brand"` (default) the icon
 * colour is set inline from the matching `--tb-provider-*` token, so the mark
...
const PROVIDER_TOKEN: Record<Provider, string> = {
  claude: "var(--tb-provider-claude)",
  gemini: "var(--tb-provider-gemini)",
...
export interface ProviderIconProps {
  provider: Provider;
  size?: number;
  /** `'brand'` (default) tints with the provider token; `'current'` inherits. */
...
export function ProviderIcon({
  ...
  const style: CSSProperties | undefined =
    tone === "brand" ? { color: PROVIDER_TOKEN[provider] } : undefined;
  const common = {
    width: size,
    height: size,
    viewBox: "0 0 24 24",
    fill: "currentColor",
    "aria-hidden": true,
    className: cn("tb-shrink-0", className),
    style,
  } as const;

  switch (provider) {
    case "claude":
      // Anthropic / Claude mark — stylised "A" sunburst (simple-icons style).
      return (
        <svg {...common}>
    ...
    case "opencode":
      // OpenCode — terminal-style chevron + underscore (approximation).
      return (
        <svg
          width={size}
          height={size}

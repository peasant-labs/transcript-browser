// Toggles the sibling workspace package in/out of this package's
// devDependencies around `npm pack`.
//
// The workspace dep (`@peasant-labs/types`) is needed at BUILD time (tsup
// inlines it into dist), but its `workspace:*` specifier is meaningless to an
// external `npm`/`file:` consumer and must NOT survive in the published
// manifest. `prepack` strips it; `postpack` restores it so the monorepo keeps
// resolving it via pnpm symlinks.
//
